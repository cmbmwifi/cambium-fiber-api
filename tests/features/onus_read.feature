...
Feature: ONU Read Operations
  ...
  Scenario: List all ONUs across all OLTs
    When I request the list of all ONUs
    Then the response status code should be 200
    And the response should be a JSON array
    And each ONU should have a "serial" field
    And each ONU should have an "olt_id" field

  Scenario: Pagination works for ONU list
  ...
  Scenario: Get ONU by serial number
    ...
    And the response should have an "olt_id" field

  Scenario: List ONUs for specific OLT
    When I get a random OLT ID from the list
    And I request the ONUs for that OLT
    Then the response status code should be 200
    And the response should be a JSON array
    And all ONUs should belong to that OLT ID

  Scenario: Get ONU from specific OLT by serial
    When I get a random OLT ID from the list
  ...
  Scenario: Non-existent ONU returns 404
